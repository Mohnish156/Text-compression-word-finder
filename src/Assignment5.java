...
public class Assignment5 {
	...
	private boolean Brute_Force = true;
	private boolean BoyerMoore = false;
	...
	private void initialise() {
		...
		searchField.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				//Ngrams n  = new Ngrams(text);
				//n.findCharProbs(pattern);
				...
				if(KMP){
					  if(pattern.length()!=1) {
						  ...
						  System.out.println("pattern length is 1 so switching to brute force");
						  BruteForce B = new BruteForce();
						  index = B.search_bruteforce(pattern,text);
					  }
				}else if(Brute_Force){
					...
					if(pattern.length() !=1) {
						...
						BoyerMoore M = new BoyerMoore();
						index = M.search(text, pattern);
					}else{
						System.out.println("pattern length is 1 so switching to brute force");
						BruteForce B = new BruteForce();
						index = B.search_bruteforce(pattern,text);
